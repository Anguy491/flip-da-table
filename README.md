...
An online table for UNO, Da Vinci Code, Las Vegas, and Conquer Westeros, built with Spring Boot + React. The backend provides authentication, sessions, game runtimes, and real-time transport. The frontend uses a repository-owned neon 8-bit arcade design system across authentication, lobby, every game, and the final scoreboard. Deployable via Docker Compose (PostgreSQL + backend + nginx-served frontend).
...
- Realtime: UNO SSE; lobby, DVC, Las Vegas, and Conquer Westeros WebSocket + STOMP
...
backend/
  ...
  src/main/java/com/flip/backend/
    ...
    conquerwesteros/    # Conquer Westeros catalog, events, phases, snapshots, views
  src/main/resources/
    application.yml
    db/migration/
frontend/
  package.json
  src/
    api/                # fetch wrappers for auth, sessions, and game commands
    context/            # AuthContext
    hooks/              # Per-game network/state containers
    components/         # Shared arcade UI + pure game views
      ...
      conquerwesteros/  # Conquer Westeros presentation & interaction
...
- Conquer Westeros runtime:
  - `GET /api/games/conquer-westeros/{gameId}/view` authenticated player perspective
  - `POST /api/games/conquer-westeros/{gameId}/commands` version-protected `ROLL_DICE`, `COMPLETE_LINE`, and `LOSE_DIE` commands
  - `/topic/conquer-westeros/{gameId}/{playerId}` private views and `/events` public animation/log events
...
## Frontend Overview
- Pages in `src/pages`: Login, Register, Forgot/Reset Password, Google callback/linking, Privacy, Dashboard, Lobby, all four games, and SessionSummary
...
- Per-game hooks own network/state concerns; deterministic view components remain fixture-testable in isolation
...
The interface follows a dark arcade-cabinet shell with neutral and game-specific semantic contexts. Conquer Westeros deliberately stays in the neutral shell and uses original geometric pixel heraldry. Press Start 2P is limited to short display text; Inter remains the body face. Components use 2px pixel borders, 4px hard shadows, visible yellow focus, short motion, and reduced-motion fallbacks.
...
## Realtime Flow
1. Initial load fetches the authenticated player's `/view` snapshot.
2. The client opens the game's SSE or STOMP channels for public and, where applicable, private updates.
3. The player submits a REST command with the game's required concurrency metadata.
4. The backend applies queued events, commits the authoritative snapshot, broadcasts after commit, and the client reconciles or reloads on conflict.
